...
根据牛顿第二定律 F = ma，质点的加速度 a = F/m。在离散时间步 dt 内，通过数值积分更新速度 v 和位置 x。

| 方法 | 位置更新 | 速度更新 | 稳定性 |
|------|----------|----------|--------|
| 显式欧拉 | x(t+1) = x(t) + v(t) * dt | v(t+1) = v(t) + a(t) * dt | 不稳定，易发散 |
| 半隐式欧拉 | x(t+1) = x(t) + v(t+1) * dt | v(t+1) = v(t) + a(t) * dt | 较稳定，常用 |
| 隐式欧拉 | x(t+1) = x(t) + v(t+1) * dt | v(t+1) = v(t) + a(t+1) * dt | 最稳定，有阻尼效果 |
...
| 弹簧类型 | 连接方式 | 作用 | 刚度系数 |
|----------|----------|------|----------|
| 结构弹簧（Structural） | 水平/垂直相邻质点 | 维持布料基本形状 | ks = 10000 |
| 剪切弹簧（Shear） | 对角相邻质点 | 防止剪切变形 | k_shear = 5000 |
| 弯曲弹簧（Bending） | 隔一个质点的水平/垂直连接 | 控制弯曲程度 | k_bending = 1000 |
...
**阻尼系数 kd 的影响**：
- kd = 1.0：布料摆动幅度较大，衰减较慢，视觉效果自然
- kd = 5.0：布料摆动迅速衰减，很快达到静止状态
...
1. **Kernel 合并**：将受力计算和位置/速度更新合并在同一个 `@ti.kernel` 中，减少 Kernel 启动开销
2. **内联函数**：使用 `ti.func` 声明辅助函数，编译时内联，减少函数调用开销
3. **原子操作**：使用 `ti.atomic_add` 处理多线程写入冲突
4. **静态循环展开**：`ti.static(range(3))` 在编译时展开循环，提高执行效率

### 🛡️ 7.2 数值稳定性处理

1. **速度钳制**：限制最大速度防止数值爆炸
2. **阻尼力**：消耗系统能量，防止能量累积
3. **隐式迭代**：使用未来状态反馈，提高稳定性
...
**提交仓库**：[https://github.com/zmx0128/cg_lab7](https://github.com/zmx0128/cg_lab7)
